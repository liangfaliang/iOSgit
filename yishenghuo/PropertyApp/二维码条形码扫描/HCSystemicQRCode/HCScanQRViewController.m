...
- (AVCaptureSession *)session {
    if (!_session) {
        ...
        LFLog(@"%f",kBgImgY/ScreenHeight);
        LFLog(@"%f",kBgImgX/ScreenWidth);
        LFLog(@"%f",kBgImgWidth/ScreenHeight);
        LFLog(@"%f",kBgImgWidth/ScreenWidth);
        CGSize size = SCREEN.size;
        CGFloat p1 = size.height/size.width;
        CGFloat p2 = 1920./1080.;  //使用了1080p的图像输出
        CGRect rect;
        if (p1 < p2) {
            CGFloat fixHeight = SCREEN.size.width * 1920. / 1080.;
            CGFloat fixPadding = (fixHeight - size.height)/2;
            rect = CGRectMake((kBgImgY + fixPadding)/ScreenHeight, kBgImgX/ScreenWidth, kBgImgWidth/ScreenHeight, kBgImgWidth/ScreenWidth);
           
        } else {
            CGFloat fixWidth = SCREEN.size.height * 1080. / 1920.;
            CGFloat fixPadding = (fixWidth - size.width)/2;
            rect = CGRectMake(kBgImgY/ScreenHeight, (kBgImgX + fixPadding)/ScreenWidth, kBgImgWidth/ScreenHeight, kBgImgWidth/ScreenWidth);
                    }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    for (CIQRCodeFeature *result in feature) {
        ...
        {
            NSInteger max = [urlStr length];
            char *nbytes = malloc(max + 1);
            for (int i = 0; i < max; i++)
            {
                unichar ch = [urlStr  characterAtIndex: i];
                nbytes[i] = (char) ch;
            }
            nbytes[max] = '\0';
            NSStringEncoding enc = CFStringConvertEncodingToNSStringEncoding(              kCFStringEncodingGB_18030_2000);
          NSString *  resultstr=[NSString stringWithCString: nbytes
                                      encoding: enc];
