//
//  GovernmentPostDetailViewController.m
//  PropertyApp
//
...
@interface GovernmentPostDetailViewController ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate,HPGrowingTextViewDelegate,UIActionSheetDelegate,UIGestureRecognizerDelegate>
...
@implementation GovernmentPostDetailViewController
...
-(void)createAlertview{
    if (self.alertview == nil) {
        ...
        self.alertTableView = [[baseTableview alloc]initWithFrame:CGRectMake(0, SCREEN.size.height - self.nameArray.count * 50, SCREEN.size.width, self.nameArray.count * 50) style:UITableViewStylePlain];
    ...
    self.alertTableView.frame = CGRectMake(0, SCREEN.size.height - self.nameArray.count * 50, SCREEN.size.width, self.nameArray.count * 50);
...
- (void)creatBottomview
{
    ...
    if (self.model) {
        if ([self.model.reply isEqualToString:@"0"]) {
            self.layView.y = SCREEN.size.height;
        }
    }
...
- (void)creatTableview
{
    ...
    if (self.tableview == nil) {
        self.tableview = [[baseTableview alloc]initWithFrame:CGRectMake(0, 0, SCREEN.size.width, SCREEN.size.height-50) style:UITableViewStylePlain];
...
- (void)requestreviewcontentData:(NSString *)articleid  parentid:(NSString *)parentid content:(NSString *)content{
    ...
    LFLog(@"用户评论dt:%@",dt);
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,GovernmentReplyUrl) params:dt success:^(id response) {
...
- (void)requestreviewData:(NSInteger)pagenum{
    ...
    LFLog(@"self.detailID:%@",self.detailID);
    ...
    LFLog(@"评论列表dt:%@",dt);
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,GovernmentReplyListUrl) params:dt success:^(id response) {
        [_tableview.mj_header endRefreshing];
        [_tableview.mj_footer endRefreshing];
        LFLog(@"评论列表:%@",response);
        NSString *str = [NSString stringWithFormat:@"%@",response[@"status"][@"succeed"]];
...
- (void)requestReport:(NSString *)report_type{
    ...
    LFLog(@"举报dt：%@",dt);
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,GovernmentReportPostUrl) params:dt success:^(id response) {
        [self.tableview.mj_header endRefreshing];
        LFLog(@"举报：%@",response);
        NSString *str = [NSString stringWithFormat:@"%@",response[@"status"][@"succeed"]];
        if ([str isEqualToString:@"1"]) {
...
- (void)requestReportType{
    ...
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,GovernmentReportPostTypeUrl) params:dt success:^(id response) {
        [self.tableview.mj_header endRefreshing];
        LFLog(@"举报：%@",response);
...
- (void)requestDetialData{
    ...
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,GovernmentPostDetailUrl) params:dt success:^(id response) {
        [_tableview.mj_header endRefreshing];
        [_tableview.mj_footer endRefreshing];
        NSString *str = [NSString stringWithFormat:@"%@",response[@"status"][@"succeed"]];
        if ([str isEqualToString:@"1"]) {
...
- (void)DeleteThePost{
    ...
    [dt setObject:self.model.id forKey:@"id"];
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,GovernmentDeletePostUrl) params:dt success:^(id response) {
        [self.tableview.mj_header endRefreshing];
        LFLog(@"删除：%@",response);
        NSString *str = [NSString stringWithFormat:@"%@",response[@"status"][@"succeed"]];
        if ([str isEqualToString:@"1"]) {
            [self presentLoadingTips:@"删除成功~~"];
            if ([self.delegate respondsToSelector:@selector(GovernmentDeletePost:isDelete:)]) {
                [self.delegate GovernmentDeletePost:self.model isDelete:YES];
            }
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    if ([self.delegate respondsToSelector:@selector(GovernmentDeletePost:isDelete:)]) {
        [self.delegate GovernmentDeletePost:self.model isDelete:NO];
    }
}
