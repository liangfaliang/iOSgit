...
- (void)AddSegumentArray:(NSArray *)SegumentArray{
    ...
    for (int i = 0; i < SegumentArray.count; i++) {
        ...
        if (i == 0) {
            buttonDownView =[[UIView alloc]initWithFrame:CGRectMake(20 +i * witdthFloat, self.bounds.size.height, witdthFloat-40, 2)];
...
-(void)selectTheSegument:(NSInteger)segument{
    ...
    if (selectSeugment != segument) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [buttonDownView setFrame:CGRectMake(20+segument * witdthFloat,self.bounds.size.height , witdthFloat-40, 2)];
        }];
