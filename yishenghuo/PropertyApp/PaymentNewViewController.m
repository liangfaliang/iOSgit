...
@interface PaymentNewViewController ()<SPPageMenuDelegate, UIScrollViewDelegate>
...
@implementation PaymentNewViewController
...
- (NSMutableArray *)dataArr {
    ...
    if (!_dataArr) {
        ...
        [_dataArr addObjectsFromArray:@[@"当期费用",@"半年预付",@"一年预付",@"已支付"]];
    }
    return _dataArr;
}
...
-(SPPageMenu *)pageMenu{
    if (!_pageMenu) {
        ...
        UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, NaviH + 40, screenW, screenH - NaviH -40)];
...
-(void)RefreshCategoryData{
    ...
    if (self.pageMenu.selectedItemIndex < self.myChildViewControllers.count) {
        PaymentListViewController *baseVc = self.myChildViewControllers[self.pageMenu.selectedItemIndex];
        [_scrollView addSubview:baseVc.view];
        baseVc.view.frame = CGRectMake(screenW*self.pageMenu.selectedItemIndex, 0, screenW, _scrollView.height);
