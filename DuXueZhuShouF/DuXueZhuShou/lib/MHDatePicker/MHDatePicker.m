...
- (void)confirmBtnClick:(id)sender
{
    ...
    if (self.Displaystr == nil) {
        self.Displaystr = @"yyyy-MM-dd HH:mm:ss";
    }
    [dateFormatter setDateFormat:self.Displaystr];
    NSString *destDateString = [dateFormatter stringFromDate:_selectDate];
    if (self.delegate && [_delegate respondsToSelector:@selector(timeString:)]) {
        [self.delegate timeString:destDateString];
    }
    if (self.delegate && [_delegate respondsToSelector:@selector(timeString:timeString:date:)]) {
        [self.delegate timeString:self timeString:destDateString date:_selectDate];
    }
...
- (void)pushDatePicker
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.pickerView.frame = CGRectMake(0, kWinH - kPVH, kWinW, kPVH);
...
- (void)dismissDatePicker
{
    //__weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:0.2 animations:^{
        self.pickerView.frame = CGRectMake(0, kWinH, kWinW, kPVH);
