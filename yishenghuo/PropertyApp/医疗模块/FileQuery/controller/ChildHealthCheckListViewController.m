//
//  ChildHealthCheckListViewController.m
//  PropertyApp
//
...
#import "ChildHealthCheckListViewController.h"
#import "FileIListTableViewCell.h"
#import "ChildHealthCheckViewController.h"
@interface ChildHealthCheckListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ChildHealthCheckListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationBarTitle = @"儿童健康检查列表";
    self.index = 0;
    [self initTableView];
    [self UpData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.nameLb.text = model[@"bm_name"];
    cell.timeLb.text = model[@"bm_monthOld"];
    return cell;
}

-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{//个人基本信息表
    NSDictionary *model = self.dataArray[indexPath.row];
    ChildHealthCheckViewController *VC = [[ChildHealthCheckViewController alloc]init];
    VC.tr_InterfaceID = @"89";
    VC.archive_no = model[@"hp_no"];
    VC.bm_id = model[@"bm_id"];
