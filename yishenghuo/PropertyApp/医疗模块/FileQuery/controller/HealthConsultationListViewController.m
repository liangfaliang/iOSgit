//
//  HealthConsultationListViewController.m
//  PropertyApp
//
...
#import "HealthConsultationListViewController.h"
#import "FileIListTableViewCell.h"
#import "HealthConsultationViewController.h"
@interface HealthConsultationListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation HealthConsultationListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationBarTitle = @"会诊记录列表";
    self.index = 0;
    [self initTableView];
    [self UpData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.timeLb.text = model[@"hg_recodate"];
    return cell;
}

-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{//个人基本信息表
    NSDictionary *model = self.dataArray[indexPath.row];
    HealthConsultationViewController *VC = [[HealthConsultationViewController alloc]init];
