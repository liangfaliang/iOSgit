...
#import "TwoWayINListViewController.h"
#import "FileIListTableViewCell.h"
#import "TwoWayINViewController.h"
@interface TwoWayINListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation TwoWayINListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    if ([self.tr_InterfaceID isEqualToString:@"97"]) {
        self.navigationBarTitle = @"双向转诊(转出)单列表";
    }else if ([self.tr_InterfaceID isEqualToString:@"98"]){
        self.navigationBarTitle = @"双向转诊(转回)单列表";
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.timeLb.text = model[@"hb_date"];
    return cell;
}

-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{//个人基本信息表
    NSDictionary *model = self.dataArray[indexPath.row];
    TwoWayINViewController *VC = [[TwoWayINViewController alloc]init];
    VC.tr_InterfaceID = self.tr_InterfaceID;
