...
+ (instancetype)actionWithTitle:(id )title style:(TYAlertActionStyle)style handler:(void (^)(TYAlertAction *))handler
{
    return [[self alloc]initWithTitle:title style:style handler:handler];
}

- (instancetype)initWithTitle:(id )title style:(TYAlertActionStyle)style handler:(void (^)(TYAlertAction *))handler
{
    if (self = [super init]) {
        _title = title;
        _style = style;
...
- (void)addTextLabels
{
    ...
    messageLabel.textColor = [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0];
...
- (void)addAction:(TYAlertAction *)action
{
    ...
    if ([action.title isKindOfClass:[NSString class]]) {
    ...
    }else if ([action.title isKindOfClass:[UIImage class]]) {
        [button setImage:(UIImage *)action.title forState:UIControlStateNormal];
    }
