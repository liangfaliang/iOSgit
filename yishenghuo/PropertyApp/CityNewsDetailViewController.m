//
//  CityNewsDetailViewController.m
//  PropertyApp
//
...
@interface CityNewsDetailViewController ()<UITableViewDelegate,UITableViewDataSource,UIWebViewDelegate,WKUIDelegate,WKNavigationDelegate>
...
@implementation CityNewsDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationBarTitle = @"新闻详情";
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.todaytableview) {
        ...
        detial.detailid = self.todayArray[indexPath.row][@"id"];
...
-(void)requestData{
    ...
    LFLog(@"城市新闻dt:%@",dt);
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,CityNewsdetailUrl) params:dt success:^(id response) {
        [_tableview.mj_header endRefreshing];
        LFLog(@"城市新闻详情:%@",response);
...
-(void)createheaerview{
    ...
    if (self.dataArray.count) {
        ...
        CGSize size = [dt[@"title"] selfadapUifont:[UIFont fontWithName:@"Helvetica-Bold" size:15] weith:20];
        ...
        authorLabel.textColor = JHColor(33, 148, 255);
