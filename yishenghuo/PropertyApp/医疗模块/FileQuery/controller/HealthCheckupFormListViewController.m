//
//  HealthCheckupFormListViewController.m
//  PropertyApp
//
...
@interface HealthCheckupFormListViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *tableView;
@property(nonatomic,strong)NSMutableArray *dataArray;
@property(nonatomic,assign)NSInteger index;
@end

@implementation HealthCheckupFormListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationBarTitle = @"健康体检列表";
    self.index = 0;
    [self initTableView];
    [self UpData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.nameLb.text = model[@"hf_name"];
    cell.timeLb.text = model[@"hf_checkDate"];
    return cell;
}

-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{//个人基本信息表
    NSDictionary *model = self.dataArray[indexPath.row];
    HealthCheckupFormViewController *VC = [[HealthCheckupFormViewController alloc]init];
    VC.tr_InterfaceID = @"84";
