...
//
//  ChanQianFollowListViewController.m
//  PropertyApp
//
...
@interface ChanQianFollowListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ChanQianFollowListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationBarTitle = @"产前随访服务列表";
    self.index = 0;
    [self initTableView];
    [self UpData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.timeLb.text = model[@"hc_FollowUpDate"];
    return cell;
}

-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{//个人基本信息表
    NSDictionary *model = self.dataArray[indexPath.row];
    ChanQianFollowViewController *VC = [[ChanQianFollowViewController alloc]init];
