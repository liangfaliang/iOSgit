...
#define tagBaseValue 5000
#define scrollViewContentOffset @"contentOffset"

#define maxTextScale 0.3
@interface BageLb : UILabel

@end

@implementation BageLb
- (void)drawRect:(CGRect)rect{
    CGFloat radius = CGRectGetWidth(self.frame)/2 > 0 ? CGRectGetWidth(self.frame)/2 : 8;
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextSetRGBFillColor (context,  1, 0, 0, 1.0);//设置填充颜色
    CGContextAddArc(context, CGRectGetWidth(self.frame)/2, CGRectGetHeight(self.frame)/2, radius, 0, M_PI * 2, 0);
...
@property (nonatomic, strong) BageLb * bagelb;
@property (nonatomic, assign) CGFloat imageRatio;
@property (nonatomic, assign) SPItemImagePosition imagePosition;

@property (nonatomic, retain)SPitemModel * smodel;
@end
...
- (void)initialize {
    ...
    _bagelb = [[BageLb alloc]init];
    _bagelb.font = [UIFont systemFontOfSize:8];
    _bagelb.text = @" ";
    _bagelb.hidden = YES;
    [self addSubview:self.titleLabel];
    [self addSubview:_bagelb];
    [_bagelb mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.titleLabel.mas_right);
        make.bottom.equalTo(self.titleLabel.mas_top).offset(4);
        make.height.mas_equalTo(_bagelb.mas_width).multipliedBy(1);
    }];
}
...
@property (nonatomic, strong) NSMutableArray *vlineArr;
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    for (int i = 0; i < items.count; i++) {
        ...
        NSAssert([object isKindOfClass:[NSString class]] || [object isKindOfClass:[UIImage class]] || [object isKindOfClass:[NSAttributedString class]] || [object isKindOfClass:[SPitemModel class]], @"items中的元素只能是NSString或UIImage类型");
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        [button setAttributedTitle:object forState:UIControlStateNormal];
    }else if ([object isKindOfClass:[SPitemModel class]]) {
        SPitemModel * model =  (SPitemModel *)object;
        if (model.badge) {
            button.bagelb.hidden = NO;
        }else{
            button.bagelb.hidden = YES;
        }
        button.smodel = model;
...
- (void)initialize {
    ...
    dividingLine.backgroundColor = self.SPPageMenuLineColor ? self.SPPageMenuLineColor : [UIColor grayColor];
...
/*
 刷新角标状态 index小于0 刷新全部
 */
-(void)RefreshBtnBadge:(NSInteger )index{
    if (index < 0) {
        for (SPItem *button in self.buttons) {
            if (button.smodel) {
                button.bagelb.hidden = !button.smodel.badge;
            }

        }
    }else if (index < self.buttons.count){
        SPItem *btn = self.buttons[index];
        if (btn.smodel) {
            btn.bagelb.hidden = !btn.smodel.badge;
        }
    }
...
- (void)buttonInPageMenuClicked:(SPItem *)sender {
    if (_items.count > sender.tag - tagBaseValue && [_items[sender.tag - tagBaseValue] isKindOfClass:[SPitemModel class]]) {
//        SPitemModel *model = _items[sender.tag - tagBaseValue];
//        model.badge = nil;
//        sender.bagelb.hidden = YES;
    }
    if (sender.titleLabel.attributedText) {
        NSMutableAttributedString *Select = [[NSMutableAttributedString alloc]initWithAttributedString:self.selectedButton.titleLabel.attributedText];
        Select.yy_color = _unSelectedItemTitleColor;
        [self.selectedButton setAttributedTitle:Select forState:UIControlStateNormal];
        
        NSMutableAttributedString *text = [[NSMutableAttributedString alloc]initWithAttributedString:sender.titleLabel.attributedText];
        text.yy_color = _selectedItemTitleColor;
        [sender setAttributedTitle:text forState:UIControlStateNormal];
    }else{
...
-(void)setSPPageMenuLineColor:(UIColor *)SPPageMenuLineColor{
    _SPPageMenuLineColor = SPPageMenuLineColor;
    _dividingLine.backgroundColor = SPPageMenuLineColor;
}
...
-(NSMutableArray *)vlineArr{
    if (_vlineArr == nil) {
        _vlineArr  =[NSMutableArray array];
    }
    return _vlineArr;
}
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        if (button.titleLabel.attributedText){
            NSMutableAttributedString *text = [[NSMutableAttributedString alloc]initWithAttributedString:button.titleLabel.attributedText];
            textW =  [text selfadaption:itemScrollViewH].width;
        }else if (button.titleLabel.text) {
            textW = [button.titleLabel.text boundingRectWithSize:CGSizeMake(MAXFLOAT, itemScrollViewH) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:_itemTitleFont} context:nil].size.width;
        }
    ...
    [self.buttons enumerateObjectsUsingBlock:^(SPItem *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ( idx < self.buttons.count -1) {
            if (self.VerticalLine) {
                UIImageView *vline = nil;
                if (self.vlineArr.count > idx) {
                    vline = self.vlineArr[idx];
                    
                }else{
                    vline = [[UIImageView alloc]initWithFrame:self.VerticalLine.frame];
                    vline.image = self.VerticalLine.image;
                    vline.backgroundColor = self.VerticalLine.backgroundColor;
                    [self.vlineArr addObject:vline];
                }
                
                vline.center = CGPointMake(lastButtonMaxX + _itemPadding/2, button.center_X);
                [button.superview addSubview:vline];
            }
        }
