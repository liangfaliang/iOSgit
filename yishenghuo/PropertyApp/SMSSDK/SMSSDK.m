                              ...
                              success:(void (^)(id response))success failure:(void (^)(NSError *error))failure
{
    ...
    LFLog(@"短信验证发送dt:%@",dt);
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,SMSsendUrl) params:dt success:^(id response) {
        ...
        if ([str isEqualToString:@"1"]) {
            ...
            hud.detailsLabelText = NSLocalizedString(response[@"data"][@"msg"], @"HUD title");
...
- (void) commitVerificationCode:(NSString *)code
    ...
    LFLog(@"短信验证dt:%@",dt);
    [LFLHttpTool post:NSStringWithFormat(ZJShopBaseUrl,[self.method isEqualToString:@"search_archives"] ? ActivateFileQueryUrl: SMSVerificationUrl) params:dt success:^(id response) {
        LFLog(@"短信验证:%@",response);
        if (success) {
            success(response);
        }
