...
- (NSInteger)goodsNumber{
    ...
    if (_goodsArr.count) {
        [_goodsArr enumerateObjectsUsingBlock:^(GNRGoodsModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            number += obj.goods_number.integerValue;
        }];
    }
    return number;
}
...
- (float)totalPrice{
    ...
    [_goodsArr enumerateObjectsUsingBlock:^(GNRGoodsModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        price += obj.goods_price.floatValue * obj.goods_number.integerValue;
    }];
    return price;
}
...
- (void)updateGoods:(GNRGoodsModel *)goods{
    NSInteger number = goods.goods_number.integerValue;//购物袋中商品的数量
    if (number>0) {
