//
//  HealthReceiveRecordListViewController.m
//  PropertyApp
//
...
@interface HealthReceiveRecordListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation HealthReceiveRecordListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationBarTitle = @"接诊记录列表";
    self.index = 0;
    [self initTableView];
    [self UpData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.timeLb.text = model[@"hr_Date"];
    return cell;
}

-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{//个人基本信息表
    NSDictionary *model = self.dataArray[indexPath.row];
    HealthReceiveRecordViewController *VC = [[HealthReceiveRecordViewController alloc]init];
